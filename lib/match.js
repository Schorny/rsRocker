...
rsR.match.updateRegionInfo = function(game, ticks) {
    var stats = rsR.match.event.getAllRegionStats(game, ticks);

    rsR.match.doUpdateRegionInfo(stats);
};

rsR.match.doUpdateRegionInfo = function(stats) {
    for(var key in stats) {
        $('#region_'+key).html(stats[key].getAsTable());
    }
};
...
rsR.match.handleMatch = function() {
    ...
    $.when(rsR.match.createHomeTeam($e), rsR.match.createAwayTeam($e)).done(function(home, away) {
        ...
        var matchlog =  new rsR.ui.MatchLog();
        new rsR.ui.Frame({name: i18n.t("ui:matchlog"), content: matchlog});

        var regioninfo =  new rsR.ui.MatchRegionInfo();
        new rsR.ui.Frame({name: i18n.t("ui:matchregioninfo"), content: regioninfo});

        var events = window.events;
        var original_do_event = window.player.do_event;
        window.player.do_event = function(minute) {
            regioninfo.update(rsR.match.event.getAllRegionStats(game, minute));
            if(events[minute]) {
